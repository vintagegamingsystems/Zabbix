#! /bin/bash
#
# This needs to be rewritten, big time it has so much redundant code within it. 
# Plus it runs the script many times over. It is very inefficient.
#
# Name: zapache
#
...
VAR=$(wget -qO- --no-check-certificate http://$HOSTNAME/server-status?auto)
    CASE_VALUE=$1
elif [[ $# == 2 ]];then
